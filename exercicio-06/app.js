...
*/
let makerTime = new Date
let horaAtual = makerTime.getHours()
...
*/
const myAge = 32
if (myAge <= 7 || myAge >= 65) { console.log('Para você, a entrada é grátis!') }
...
console.log(numbersFiltered.toString())
...
let totalBooleans = 0, totalNumbers = 0, totalStrings = 0

console.log(typeof (crazyArray[0]), typeof (crazyArray[1]), typeof (crazyArray[2]))

for (let i = 0; i < crazyArray.length; i++) {
  if (typeof (crazyArray[i]) == 'boolean') { totalBooleans++ }
  if (typeof (crazyArray[i]) == 'string') { totalStrings++ }
  if (typeof (crazyArray[i]) == 'number') { totalNumbers++ }
}

console.log(`O crazyArray tem ${totalBooleans} booleans, ${totalNumbers} números e ${totalStrings} strings.`)
/*
  06
...
for (let i = 0; i < randomNumbers.length; i++) {
  if (randomNumbers[i] % 2 == 0) { arrayPar.push(randomNumbers[i]) }
  else { arrayImpar.push(randomNumbers[i]) }
}

console.log(`Numeros ímpares: ${arrayImpar.join(", ")}. Números pares: ${arrayPar.join(", ")}.`)
